...
import ReactTooltip from 'react-tooltip'; //for hover 
...
const PlayerInfo = ({ player, playerIcon, mt }) => {
    return (
        <div className={`${styles.flexCenter} ${mt ? 'mt-4' : 'mb-4'}`}>
            <img data-for={`Player-${mt ? '1' : '2'}`} data-tip src={playerIcon} alt="player02" className="w-14  object-contain rounded-full" />
            <div
                data-for={`Health-${mt ? '1' : '2'}`} //used for tooltip as id
            ...
            <ReactTooltip id={`Player-${mt ? '1' : '2'}`} effect="solid" backgroundColor="#7f46f0">
                ...
                <p className={styles.playerInfo}>
                    <span className={styles.playerInfoSpan}>Address:</span> {`${player?.playerAddress?.slice(0, 5)}...${player?.playerAddress?.slice(38, 42)}`}
                </p>
            </ReactTooltip>
